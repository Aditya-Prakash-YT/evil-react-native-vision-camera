...
internal fun CameraSession.configureOutputs(configuration: CameraConfiguration) {
  ...
  if (previewConfig != null) {
    ...
    previewSurfaceProvider = previewConfig.config.surfaceProvider
  } else {
    previewOutput = null
  }
...
internal suspend fun CameraSession.configureCamera(provider: ProcessCameraProvider, configuration: CameraConfiguration) {
  ...
  val cameraXInfo = provider.availableCameraInfos.find { it.id == cameraId }
  if (cameraXInfo == null) {
      Log.i(CameraSession.TAG, "Camera #$cameraId not found in CameraX, using Evil Session...")
      val surfaceProvider = previewSurfaceProvider ?: throw Error("Preview Surface Provider not found!")
      evilSession?.close()
      evilSession = EvilCameraSession(cameraManager, cameraId, surfaceProvider, callback)
      return
  } else {
      evilSession?.close()
      evilSession = null
