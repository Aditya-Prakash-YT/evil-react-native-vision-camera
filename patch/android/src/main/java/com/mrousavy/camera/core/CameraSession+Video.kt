...
fun CameraSession.startRecording(
  ...
  val evilSession = evilSession
  if (evilSession != null) {
      evilSession.startRecording(options, { video ->
          callback(video)
      }, { error ->
          onError(error as? CameraError ?: CameraError("capture", "unknown", error.message, error))
      })
      return
  }
